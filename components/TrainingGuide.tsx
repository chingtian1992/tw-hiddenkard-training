...
const TrainingGuide: React.FC = () => {
  ...
  const handleDescClick = (song: Song) => {
    // 優先顯示自訂說明，若無則顯示預設
    if (song.description) {
  ...
  const handleChartClick = (url?: string) => {
  ...
  return (
    <div className="p-4 pb-32">
      ...
      <div className="grid grid-cols-1 gap-4">
        {SONGS.map(song => {
          // 檢查是否有空耳或圖卡資源
          const hasPhonetic = song.phoneticUrl && song.phoneticUrl !== '';
          const hasChart = song.chartUrl && song.chartUrl !== '';
          const hasResources = hasPhonetic || hasChart;
                ...
                <div className="flex gap-1.5">
                  {song.isChallenge && hasResources ? (
                    <>
                      ...
                      <button 
                        onClick={() => handleChartClick(song.chartUrl)}
                        className="flex-1 bg-white/5 border border-white/10 text-white text-[9px] py-2 rounded-xl font-black uppercase tracking-tighter hover:bg-white/10 active:scale-95 transition-all"
                      >
                        圖卡
                      </button>
                    ...
                    <button 
                      onClick={() => handleDescClick(song)}
                      className="w-full bg-transparent border border-gray-800 text-gray-500 text-[9px] py-2 rounded-xl font-black uppercase tracking-widest hover:text-white hover:border-white transition-all active:scale-95"
                    >
                      說明
                    </button>
                  )}
